...
try:
    ...
    logger.debug("Determined version %s via importlib.metadata", __version__)
except PackageNotFoundError:
    # if that fails, check for static version file written by setuptools_scm
    try:
        from .version import version as __version__
        logger.debug(
            "Determined version %s from autogenerated dtoolcore/version.py",
            __version__,
        )
...
class _BaseDataSet:
    ...
    def delete_annotation(self, annotation_name):
        ...
        :raises: DtoolCoreInvalidNameError if the annotation name
                 is invalid
        """
        self._storage_broker.delete_annotation(annotation_name)
...
class DataSet(_BaseDataSet):
    ...
    def __init__(self, uri, admin_metadata, config_path=None):
        super().__init__(uri, admin_metadata, config_path)
    ...
    def freeze_with_manifest(self, manifest, frozen_at=None):
        ...
        if manifest_items:
            ...
            existing_identifiers = set(
                dtoolcore.utils.generate_identifier(h)
        ...
        if frozen_at is not None:
            ...
            datetime_obj = datetime.datetime.now(datetime.timezone.utc)
            metadata_update["frozen_at"] = dtoolcore.utils.timestamp(
                datetime_obj
            )
